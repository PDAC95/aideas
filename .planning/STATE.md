...
**Current focus:** v1.2 Admin Dashboard — defining requirements
...
Last activity: 2026-05-04 — Milestone v1.2 started (questioning gate complete)
...
- **Carry-over from v1.1 as own phase** — Next.js 16 build blocker, AutomationSuccessRate placeholder, assertOrgMembership consolidation, symmetric reCAPTCHA bypass — bundled as one cleanup phase, ideally first to unblock build.

### Blockers/Concerns

- v1.1 build blocker: `next/dynamic ssr:false` in `web/src/app/(dashboard)/dashboard/automations/[id]/page.tsx:16` breaks `npm run build` under Next.js 16 + Turbopack. Should be addressed early in v1.2 to keep CI green during admin development.
